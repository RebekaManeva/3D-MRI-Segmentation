...
class ResidualConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch, num_convs=2):
        ...
        for i in range(num_convs):
            if i == 0:
                layers.append(nn.Conv3d(in_ch, out_ch, 5, padding=2))
            else:
                layers.append(nn.Conv3d(out_ch, out_ch, 5, padding=2))
        ...
        if in_ch != out_ch:
            self.residual = nn.Conv3d(in_ch, out_ch, 1)
        else:
...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, num_convs=2):
        ...
        self.conv_block = ResidualConvBlock(out_ch, out_ch, num_convs)
...
class UpBlock(nn.Module):
    def __init__(self, in_ch, out_ch, num_convs=2):
        ...
        self.conv_block = ResidualConvBlock(out_ch * 2, out_ch, num_convs)
...
class VNet(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, fmaps=(12, 24, 48, 96, 192)):
        super().__init__()

        self.init_conv = ResidualConvBlock(in_channels, fmaps[0], num_convs=1)
        self.down1 = DownBlock(fmaps[0], fmaps[1], num_convs=2)
        self.down2 = DownBlock(fmaps[1], fmaps[2], num_convs=3)
        self.down3 = DownBlock(fmaps[2], fmaps[3], num_convs=3)
        self.down4 = DownBlock(fmaps[3], fmaps[4], num_convs=3)

        self.up1 = UpBlock(fmaps[4], fmaps[3], num_convs=3)
        self.up2 = UpBlock(fmaps[3], fmaps[2], num_convs=3)
        self.up3 = UpBlock(fmaps[2], fmaps[1], num_convs=2)
        self.up4 = UpBlock(fmaps[1], fmaps[0], num_convs=1)

        self.out_conv = nn.Conv3d(fmaps[0], out_channels, 1)
...
def plot_metrics(train_metrics, val_metrics, save_dir):
    ...
    for metric_name in metric_names:
        if not val_metrics[metric_name] or not train_metrics[metric_name]:
            print(f"Missing data for {metric_name.upper()}.")
            continue
...
def main_train():
    ...
    model = VNet(in_channels=1, out_channels=1, fmaps=(12, 24, 48, 96, 192)).to(device)
