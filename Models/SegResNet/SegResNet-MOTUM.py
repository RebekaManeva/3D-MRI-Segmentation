...
def make_model(n_channels, n_classes=3, base_filters=32):
    return SegResNet(spatial_dims=3, init_filters=base_filters,
                     ...
                     norm='instance', dropout_prob=0.1).to(DEVICE)
...
for ep in range(1, EPOCHS + 1):
    ...
    if val_dice > best_dice:
        best_dice = val_dice
        torch.save(model.state_dict(), f"best_segresnet_{SETUP}.pth")
...
val_dices = [float(x) for x in re.findall(r"ValDice=([0-9]*\.?[0-9]+)", log_text)]

if val_dices:
    epochs = list(range(1, len(val_dices) + 1))
    plt.figure(figsize=(7,5))
    plt.plot(epochs, val_dices, marker='o', markersize=3, linewidth=1.8)
    plt.title("Validation Dice vs Epoch", fontsize=13)
