...
def merge_labels_collate(batch):
    ...
    if torch.sum(label) == 0: return None
    b["label"] = label
    b.pop("label_ce", None)
    b.pop("label_fl", None)
    return b
...
def collate_skip_empty(batch):
    ...
    return merge_labels_collate(batch)
...
def make_loss():
    ...
    weights = torch.tensor([0.05, 0.35, 0.60]).to(DEVICE)
...
if __name__ == "__main__":
    ...
    model = make_model(base_filters=BASE_FILTERS)
    loss_fn = make_loss()
